...
func getAllNpcDB(db sql.DB) string {
	sql := "select * from test.npc_data;"
	...
	row, _ := db.Query(sql)
	for row.Next() {
		row.Scan(&ID, &du, &temp_data)
		npc_data += temp_data
	}
	return npc_data
}

func getNpcDB(db sql.DB, dur string) string {
	sql := "select * from test.npc_data where n_duration = '" + dur + "';"
	var npc_data string
	var temp_data string
	var ID string
	var du string
...
func main() {
	...
	db, err := sql.Open("mysql", "root:'passward'@tcp(localhost:3306)/'db_name'")
